...
export function Alert({ alertType }) {
  const alarmText = {
  ...
  if (alarmText[alertType]) {
    return (
      <>
        <AlertBox level={alertType} message={alarmText[alertType]} />
      </>
    );
  } else {
    return (
      <>
        <AlertBox level="error" message={`Other error : ${alertType}`} />
      </>
    );
  }
}

function AlertBox({ level, message }) {
  const bgColor = {
    error: "#F9C8C8",
    warning: "#F9D9C8",
    info: "#F9EBC8",
  ...
  return (
    <div
      css={css`
        ...
        padding-right: 16px;
        padding-left: 16px;
        background-color: ${bgColor[level]};
        font-size: 16px;
        font-weight: 600;
        color: black;
        width: 650px;
